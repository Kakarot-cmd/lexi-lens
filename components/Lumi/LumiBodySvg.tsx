/**
 * components/Lumi/LumiBodySvg.tsx — procedural SVG body (Lumi v1/v2 art).
 *
 * Previously the only body. Now the fallback for the dispatcher in
 * LumiBody.tsx — used when LUMI_RIVE_ENABLED is false OR when the Rive
 * asset fails to load.
 *
 * Pure static SVG (no animated SVG attrs, no createAnimatedComponent).
 * All movement / scale / bob is handled by the OUTER <Animated.View> in
 * LumiMascot. This file's API is unchanged from the original LumiBody.tsx.
 *
 * v2 highlights:
 *   • `colorTick` prop — 0..5 index into RAINBOW_PALETTE. When
 *     `theme === 'rainbow'`, LumiMascot drives this via React state on a
 *     slow timer, swapping the gradient stop colors so Lumi cycles
 *     through hues. Plain prop change → React re-renders SVG with new
 *     fills. No Reanimated, no native shadow-node update path.
...
export function LumiBodySvg(props: LumiBodyProps): React.ReactElement {
  const {
    size = 64,
    theme = 'normal',
    mood = 'happy',
